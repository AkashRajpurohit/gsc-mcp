  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        days: { type: 'number', exclusiveMinimum: 0, description: 'Lookback window in days from yesterday (default 28). Ignored if startDate given.' },
        ...
        startDate: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'YYYY-MM-DD (overrides days and datePreset).' },
        endDate: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'YYYY-MM-DD (default yesterday).' },
        dimensions: {
          type: 'array',
        ...
        rowLimit: { type: 'number', minimum: 1, description: `Total rows to return (default ${DEFAULT_ROW_LIMIT}). Values above ${API_MAX_ROWS_PER_REQUEST} are fetched by paging automatically, up to maxRows.` },
        maxRows: { type: 'number', minimum: 1, description: `Safety ceiling on total rows fetched (default ${DEFAULT_MAX_ROWS}). Protects against accidentally huge pulls.` },
        startRow: { type: 'number', minimum: 0, description: 'Zero-based offset to start from (default 0), for manual pagination.' },
      },
      required: ['site'],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        siteUrl: { type: 'string', description: 'Alias for "site".' },
        days: { type: 'number', exclusiveMinimum: 0, description: 'Length of the current period in days ending yesterday (default 28). Ignored if startDate given.' },
        ...
        startDate: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Current period start, YYYY-MM-DD (overrides days and datePreset).' },
        endDate: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Current period end, YYYY-MM-DD (default yesterday).' },
        previousStartDate: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Previous period start, YYYY-MM-DD. Defaults to the equal-length window before the current period.' },
        previousEndDate: { type: 'string', pattern: '^\\d{4}-\\d{2}-\\d{2}$', description: 'Previous period end, YYYY-MM-DD. Required if previousStartDate is given.' },
        ...
        rowLimit: { type: 'number', minimum: 1, description: `When grouping, how many rows to fetch per period (default ${DEFAULT_COMPARE_ROW_LIMIT}).` },
        limit: { type: 'number', minimum: 1, description: `How many rows to return in largestDeclines / largestGains (default ${DEFAULT_TOP_N}).` },
      },
      required: ['site'],
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        minImpressions: { type: 'number', minimum: 0, description: `Only consider rows with at least this many impressions (default ${DEFAULT_OPPORTUNITY_MIN_IMPRESSIONS}).` },
        minPosition: { type: 'number', minimum: 1, description: `striking_distance only: lowest average position to include (default ${STRIKING_MIN_POSITION}).` },
        maxPosition: { type: 'number', minimum: 1, description: `Highest average position to include (default ${STRIKING_MAX_POSITION} for striking_distance, ${LOW_CTR_MAX_POSITION} for low_ctr).` },
        maxCtr: { type: 'number', minimum: 0, maximum: 1, description: `low_ctr only: only flag rows with CTR at or below this (0 to 1, default ${LOW_CTR_MAX_CTR}).` },
        rowLimit: { type: 'number', minimum: 1, description: `How many rows to scan (default ${DEFAULT_OPPORTUNITY_ROW_LIMIT}).` },
        limit: { type: 'number', minimum: 1, description: `Max opportunities to return (default ${DEFAULT_OPPORTUNITY_LIMIT}).` },
      },
      required: ['site'],
    },
  },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        siteUrl: { type: 'string', description: 'Alias for "site".' },
        urls: { type: 'array', items: { type: 'string' }, minItems: 1, description: 'Full URLs to inspect, e.g. ["https://example.com/a/", "https://example.com/b/"].' },
        concurrency: { type: 'number', minimum: 1, maximum: MAX_INSPECT_CONCURRENCY, description: `Max parallel requests (default ${DEFAULT_INSPECT_CONCURRENCY}, max ${MAX_INSPECT_CONCURRENCY}).` },
        maxUrls: { type: 'number', minimum: 1, description: `Safety cap on how many URLs to inspect (default ${DEFAULT_INSPECT_MAX_URLS}).` },
      },
      required: ['site', 'urls'],
    },
  },

...
export function validateInspectUrls(a) {
  if (!Array.isArray(a.urls) || a.urls.length === 0) {
    throw new ToolError('"urls" must be a non-empty array of URL strings.');
  }
  for (const url of a.urls) {
    if (typeof url !== 'string' || url.trim() === '') {
      throw new ToolError('Each entry in "urls" must be a non-empty string.');
    }
  }
  if (a.concurrency != null && (!Number.isInteger(a.concurrency) || a.concurrency < 1 || a.concurrency > MAX_INSPECT_CONCURRENCY)) {
    throw new ToolError(`"concurrency" must be an integer between 1 and ${MAX_INSPECT_CONCURRENCY}.`);
  }
  positiveInt(a, 'maxUrls');
}
